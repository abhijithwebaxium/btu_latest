...
export default function AdminAnnouncementDesk() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
            <Bell className="w-5 h-5 sm:w-6 sm:h-6 text-[#ed143d]" />
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
          {activeCount > 0 && (
            <span className="text-xs font-bold px-3 py-1.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shrink-0">
              {activeCount} Active
            </span>
          )}
          <button
            onClick={() => { setShowForm(p => !p); setErrorMsg(null) }}
            className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[#ed143d] hover:bg-rose-700 text-white text-xs sm:text-sm font-bold shadow-lg shadow-[#ed143d]/30 transition-all"
          >
      ...
      {/* Success toast */}
      {successMsg && (
        <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 flex items-center gap-3 text-xs sm:text-sm font-semibold">
      ...
      {/* Create Form */}
      {showForm && (
        <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-4 sm:p-6 shadow-xl space-y-5">
          <h3 className="text-sm sm:text-base font-bold text-white border-b border-slate-800 pb-3.5 flex items-center gap-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none focus:border-[#ed143d] focus:ring-1 focus:ring-[#ed143d]"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none focus:border-[#ed143d] focus:ring-1 focus:ring-[#ed143d] resize-none"
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs sm:text-sm text-white focus:outline-none focus:border-[#ed143d]"
                >
              ...
              <div className="flex flex-wrap gap-1.5">
                {DURATION_OPTIONS.map(opt => (
                  <button
                    ...
                    className={`px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg text-xs font-semibold border transition-all ${
            ...
            <div className="flex justify-end gap-3 pt-2 border-t border-slate-800">
              <button type="button" onClick={() => { setShowForm(false); resetForm() }} className="px-4 py-2 rounded-xl bg-slate-800 text-slate-300 hover:bg-slate-700 text-xs sm:text-sm font-semibold transition-colors">
                Cancel
              </button>
              <button type="submit" disabled={submitting} className="inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2 rounded-xl bg-[#ed143d] hover:bg-rose-700 text-white text-xs sm:text-sm font-bold shadow-lg shadow-[#ed143d]/30 transition-all disabled:opacity-50">
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-slate-800/60">
            {announcements.map(ann => {
              ...
              return (
                <div key={ann._id} className="p-4 sm:p-5 hover:bg-slate-800/30 transition-colors flex items-start gap-3 sm:gap-4">
                  <div className={`mt-1.5 w-2 h-2 rounded-full shrink-0 ${priorityDot[ann.priority]}`} />
                  <div className="flex-1 min-w-0 space-y-1.5">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="font-bold text-xs sm:text-sm text-white">{ann.title}</p>
                    ...
                    <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-[10px] sm:text-[11px] text-slate-500 pt-0.5">
